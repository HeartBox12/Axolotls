...
func _process(delta):
	target = (get_global_mouse_position() / $"..".scale.x) - Vector2(16, 16) #this is gonna be where the logic to find the nearest/first enemy in range goes, using mouse position for now
...
func _physics_process(_delta):
	fireRotation = target.angle_to_point(position) + PI #Gets the angle between turret and target in radians, adds 90 degrees/quarter turn
